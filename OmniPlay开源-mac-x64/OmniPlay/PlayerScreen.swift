...
struct PlayerScreen: View {
    ...
    var body: some View {
        ...
        .onChange(of: playerManager.currentFilename) { _, newName in
             ...
             guard let newFile = allPlaylistFiles.first(where: { $0.fileName == newName }), oldId != newFile.id else { return }
             Task.detached {
    ...
    private func resetHideTimer() { hideUITask?.cancel(); hideUITask = Task { try? await Task.sleep(nanoseconds: 1_500_000_000); guard !Task.isCancelled else { return }; await MainActor.run { withAnimation(.easeInOut(duration: 0.5)) { showControls = false } } } }
