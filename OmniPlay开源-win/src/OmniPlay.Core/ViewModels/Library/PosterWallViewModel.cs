...
public partial class PosterWallViewModel : ObservableObject
{
    ...
    public bool HasStatusMessage => ShouldShowHomeStatusMessage(StatusMessage);
    ...
    {
        var normalizedBaseUrl = PendingNetworkBaseUrl.Trim();
    ...
    private static bool ShouldShowHomeStatusMessage(string? statusMessage)
    {
        ...
        return normalized.StartsWith("正在扫描媒体源", StringComparison.Ordinal) ||
               normalized.StartsWith("正在后台扫描媒体源", StringComparison.Ordinal) ||
               normalized.StartsWith("扫描完成", StringComparison.Ordinal) ||
               normalized.StartsWith("后台扫描完成", StringComparison.Ordinal) ||
